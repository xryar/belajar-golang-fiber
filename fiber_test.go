...
func TestCtx(t *testing.T) {
	...
	request := httptest.NewRequest("GET", "/hello?name=Arya", nil)
	response, err := app.Test(request)
	assert.Nil(t, err)
	assert.Equal(t, 200, response.StatusCode)
	...
	bytes, err := io.ReadAll(response.Body)
	assert.Nil(t, err)
	assert.Equal(t, "Hello Arya", string(bytes))
...
func TestHttpRequest(t *testing.T) {
	...
	request.Header.Set("firstname", "Arya")
	request.AddCookie(&http.Cookie{Name: "lastname", Value: "Andaru"})
	response, err := app.Test(request)
	assert.Nil(t, err)
	assert.Equal(t, 200, response.StatusCode)
	...
	bytes, err := io.ReadAll(response.Body)
	assert.Nil(t, err)
	assert.Equal(t, "Hello Arya Andaru", string(bytes))
}
...
func TestRouteParameter(t *testing.T) {
	...
	request := httptest.NewRequest("GET", "/users/acumalaka/orders/10", nil)
	response, err := app.Test(request)
	assert.Nil(t, err)
	assert.Equal(t, 200, response.StatusCode)
	...
	bytes, err := io.ReadAll(response.Body)
	assert.Nil(t, err)
	assert.Equal(t, "Get Order 10 From User acumalaka", string(bytes))
}
...
func TestFormRequest(t *testing.T) {
	...
	body := strings.NewReader("name=Arya")
...
func TestRequestBody(t *testing.T) {
	...
	body := strings.NewReader(`{"username":"Arya", "password":"rahasia"}`)
...
func TestBodyParserJSON(t *testing.T) {
	...
	body := strings.NewReader(`{"username":"Arya", "password":"rahasia", "name":"Arya Rizki"}`)
	...
	bytes, err := io.ReadAll(response.Body)
	assert.Nil(t, err)
	assert.Equal(t, "Register Success Arya", string(bytes))
}
...
func TestResponseJSON(t *testing.T) {
	...
	bytes, err := io.ReadAll(response.Body)
	assert.Nil(t, err)
	assert.Equal(t, `{"name":"Arya Rizki","username":"arya"}`, string(bytes))
}
